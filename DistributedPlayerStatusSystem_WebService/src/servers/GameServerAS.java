...
public class GameServerAS extends GameServer
{	
            ...
            String pUsername, String pPassword, String pIPAddress)
	{
		...
		try
		{
			...
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Invalid GeoLocation");
				return "Error Creating Player Account : Invalid GeoLocation";
			}
			if(pAge <= 0)
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Invalid Age");
				return "Error Creating Player Account : Invalid Age";
			}
			if(pUsername.length() < 6)
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Username has to be at least 6 characters");
				return "Error Creating Player Account : Username has to be at least 6 characters";
			}
			if(pUsername.length() > 15)
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Username has to be at most 15 characters");
			...
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Username cannot start with " + pUsername.charAt(0));
				return "Error Creating Player Account : Username cannot start with " + pUsername.charAt(0);
			}
			if(getAccount(pUsername) != null)
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Username " + pUsername + " Already Exists");
				return "Error Creating Player Account : Username " + pUsername + " Already Exists";
			}
			if(pPassword.length() < 6)
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Password has to be at least 6 characters");
				return "Error Creating Player Account : Password has to be at least 6 characters";
			}
			if(!validate(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : Invalid IP-address");
				return "Error Creating Player Account : Invalid IP-address";
			}
			if(IPAddressExists(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Creating Player Account : IP-address Already Exists");
			...
			Log.info(Parameters.log_AS, "Player Account created :\nFirstName \"" +  pFirstName +  "\", LastName \"" +  pLastName + 
		...
		{
			Log.info(Parameters.log_AS, "Error Creating Player Account : Missing Parameter");
	...
	public String playerSignIn(String pUsername, String pPassword, String pIPAddress)
	{
		...
		try
		{
			...
			if((pUsername.charAt(0) > 'Z' && pUsername.charAt(0) < 'a') || pUsername.charAt(0) < 'A' || pUsername.charAt(0) > 'z')
			{
				Log.info(Parameters.log_AS, "Error Player Sign in : Username cannot start with " + pUsername.charAt(0));
				return "Error Player Sign in : Username cannot start with " + pUsername.charAt(0);
			}
			Account tmpAccount = getAccount(pUsername);
			if(tmpAccount == null)
			{
				Log.info(Parameters.log_AS, "Error Player Sign in : Username Was Not Found");
				tmpAccount = null;
				return "Error Player Sign in : Username Was Not Found";
			}
			if(!tmpAccount.getPassword().equals(pPassword))
			{
				Log.info(Parameters.log_AS, "Error Player Sign in : Incorrect Password");
				tmpAccount = null;
				return "Error Player Sign in : Incorrect Password";
			}
			if(!validate(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Player Sign in : Invalid IP-address");
				tmpAccount = null;
				return "Error Player Sign in : Invalid IP-address";
			}
			if(!tmpAccount.getIPAddress().equals(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Player Sign in : Nonmatching IP-address");
				tmpAccount = null;
			...
			if(!tmpAccount.isOnline())
			{
				...
				tmpAccount = null;
				Log.info(Parameters.log_AS, "Player Sign in : Player " +  pUsername + " Is Now Online");
				return "Player Sign in : Player " +  pUsername + " Is Now Online";
			}
			else
			{
				Log.info(Parameters.log_AS, "Error Player Sign in : Player " + pUsername + " Is Already Online");
				tmpAccount = null;
		...
		{
			Log.info(Parameters.log_AS, "Error Player Sign in : Missing Parameter");
			return "Error Player Sign in : Missing Parameter";
		}
		catch(NumberFormatException e)
		{
			Log.info(Parameters.log_AS, "Error Player Sign in : Invalid Age");
	...
	public String playerSignOut(String pUsername, String pIPAddress) 
	{
		...
		try
		{
			...
			if((pUsername.charAt(0) > 'Z' && pUsername.charAt(0) < 'a') || pUsername.charAt(0) < 'A' || pUsername.charAt(0) > 'z')
			{
				Log.info(Parameters.log_AS, "Error Player Sign out : Username cannot start with " + pUsername.charAt(0));
				return "Error Player Sign out : Username cannot start with " + pUsername.charAt(0);
			}
			Account tmpAccount = getAccount(pUsername);
			if(tmpAccount == null)
			{
				Log.info(Parameters.log_AS, "Error Player Sign out : Username Was Not Found");
				tmpAccount = null;
				return "Error Player Sign out : Username Was Not Found";
			}
			if(!validate(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Player Sign out : Invalid IP-address");
				tmpAccount = null;
				return "Error Player Sign out : Invalid IP-address";
			}
			if(!tmpAccount.getIPAddress().equals(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Player Sign out : Nonmatching IP-address");
				tmpAccount = null;
			...
			if(tmpAccount.isOnline())
			{
				...
				tmpAccount = null;
				Log.info(Parameters.log_AS, "Player Sign out : Player " +  pUsername + " Is Now Offline");
				return "Player Sign out : Player " +  pUsername + " Is Now Offline";
			}
			else
			{
				Log.info(Parameters.log_AS, "Error Player Sign out : Player " + pUsername + " Is Already Offline");
				tmpAccount = null;
		...
		{
			Log.info(Parameters.log_AS, "Error Player Sign out : Missing Parameter");
			return "Error Player Sign out : Missing Parameter";
		}
		catch(NumberFormatException e)
		{
			Log.info(Parameters.log_AS, "Error Player Sign out : Invalid Age");
	...
	public String transferAccount(String pUsername, String pPassword, String pOldIPAddress, String pNewIPAddress) 
	{
		...
		try
		{
			...
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Invalid GeoLocation");
				return "Error Transferring Account : Invalid GeoLocation";
			}
			if((pUsername.charAt(0) > 'Z' && pUsername.charAt(0) < 'a') || pUsername.charAt(0) < 'A' || pUsername.charAt(0) > 'z')
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Username cannot start with " + pUsername.charAt(0));
				return "Error Transferring Account : Username cannot start with " + pUsername.charAt(0);
			}
			Account tmpAccount = getAccount(pUsername);
			if(tmpAccount == null)
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Username Was Not Found");
				tmpAccount = null;
				return "Error Transferring Account : Username Was Not Found";
			}
			if(!tmpAccount.getPassword().equals(pPassword))
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Incorrect Password");
				tmpAccount = null;
				return "Error Transferring Account : Incorrect Password";
			}
			if(!validate(pOldIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Invalid IP-address");
				tmpAccount = null;
				return "Error Transferring Account : Invalid IP-address";
			}
			if(!tmpAccount.getIPAddress().equals(pOldIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Nonmatching IP-address");
				tmpAccount = null;
				return "Error Transferring Account : Nonmatching IP-address";
			}
			if(!validate(pNewIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : Invalid New IP-address");
				tmpAccount = null;
				...
				if(bSuccessRemoval)
				{
					...
					if(pNewIPAddress.length() >= 3 && pNewIPAddress.substring(0,3).equals(Parameters.GeoLocationOfGameServerNA))
					{
						try 
						{
							...
							requestToTransfer = new DatagramPacket(message, message.length, host, Parameters.UDPportNA);
			...
			{
				Log.info(Parameters.log_AS, "Error Transferring Account : removal faild");
				return "ERROR_TRANSFER_ACCOUNT";
			}
		}
		catch(NullPointerException e)
		{
			Log.info(Parameters.log_AS, "Error Transferring Account : Missing Parameter");
			return "Error Transferring Account : Missing Parameter";
		}
		catch(NumberFormatException e)
		{
			Log.info(Parameters.log_AS, "Error Transferring Account : Invalid Age");
	...
	public String getPlayerStatus(String pAdminUsername, String pAdminPassword, String pIPAddress)
	{
		...
		try
		{
			if(pIPAddress.length() < 3 || !pIPAddress.substring(0,3).equals(Parameters.GeoLocationOfGameServerAS))
			{
				Log.info(Parameters.log_AS, "Error Getting Player Status : Invalid GeoLocation");
				return "Error Getting Player Status : Invalid GeoLocation";
			}
			if(!pAdminUsername.equals("Admin"))
			{
				Log.info(Parameters.log_AS, "Error Getting Player Status : Incorrect Administrator Username");
				return "Error Getting Player Status : Incorrect Administrator Username";
			}
			if(!pAdminPassword.equals("Admin"))
			{
				Log.info(Parameters.log_AS, "Error Getting Player Status : Incorrect Administrator Password");
				return "Error Getting Player Status : Incorrect Administrator Password";
			}
			if(!validate(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Getting Player Status : Invalid IP-address");
			...
			try 
			{
				...
				Log.info(Parameters.log_AS, "Get Player Status : " + Parameters.serverName_AS + ": " + getNumberOfOnlinePlayer() + " online, " + getNumberOfOfflinePlayer() + 
						" offline. \n" + repliedMessage1.substring(0, 24) + "\n" + repliedMessage2.substring(0, 24));
				return "Get Player Status : " + Parameters.serverName_AS + ": " + getNumberOfOnlinePlayer() + " online, " + getNumberOfOfflinePlayer() + 
						" offline. \n" + repliedMessage1.substring(0, 24) + "\n" + repliedMessage2.substring(0, 24);
			} catch (SocketException e) {
				System.out.println("SocketException : " + e.getMessage());
			} catch (IOException e) {
		...
		{
			Log.info(Parameters.log_AS, "Error Getting Player Status : Missing Parameter");
		...
		Log.info(Parameters.log_AS, "Error Getting Player Status : Socket Excpetion");
	...
	public String suspendAccount(String pAdminUsername, String pAdminPassword, String pIPAddress, String pUsernameToSuspend) 
	{
		...
		try
		{
			if(pIPAddress.length() < 3 || !pIPAddress.substring(0,3).equals(Parameters.GeoLocationOfGameServerAS))
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : Invalid GeoLocation");
				return "Error Suspending Account : Invalid GeoLocation";
			}
			if(!pAdminUsername.equals("Admin"))
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : Incorrect Administrator Username");
				return "Error Suspending Account : Incorrect Administrator Username";
			}
			if(!pAdminPassword.equals("Admin"))
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : Incorrect Administrator Password");
				return "Error Suspending Account : Incorrect Administrator Password";
			}
			if(!validate(pIPAddress))
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : Invalid IP-address");
			...
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : Username cannot start with " + pUsernameToSuspend.charAt(0));
			...
			if(tmpAccount == null)
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : Username Was Not Found");
				tmpAccount = null;
			...
			if(bSuccess)
			{
				Log.info(Parameters.log_AS, "Player Account suspended :\nUsername \"" +  pUsernameToSuspend +  "\", Password \"" + tmpAccount.getPassword() + "\", IP-address \"" + tmpAccount.getIPAddress() + "\"");
				return "Player Account suspended :\nUsername \"" +  pUsernameToSuspend +  "\", Password \"" + tmpAccount.getPassword() + "\", IP-address \"" + tmpAccount.getIPAddress() + "\"\n";
			}
			else
			{
				Log.info(Parameters.log_AS, "Error Suspending Account : suspension faild");
				return "Error Suspending Account : suspension faild";
			}
		}
		catch(NullPointerException e)
		{
			Log.info(Parameters.log_AS, "Error Suspending Account : Missing Parameter");
			return "Error Transferring Account : Missing Parameter";
		}
		catch(NumberFormatException e)
		{
			Log.info(Parameters.log_AS, "Error Suspending Account : Invalid Age");
			return "Error Transferring Account : Invalid Age";
		}
	}
